...
use fedimint_lnurl::PayResponse;
...
pub fn parse_lnurl(lnurl: &str) -> Result<LnUrlWrapper, String> {
    ...
    if let Ok(url) = reqwest::Url::parse(&lnurl.to_lowercase()) {
        for (key, value) in url.query_pairs() {
            if key == "lightning" || key == "lnurl" {
                if let Ok(result) = parse_lnurl(&value) {
                    return Ok(result);
                }
            }
        }
    ...
    if let Some(endpoint) = fedimint_lnurl::parse_lnurl(lnurl) {
        return Ok(LnUrlWrapper(endpoint));
    }
    ...
    Err("Failed to parse lnurl".to_string())
}
...
pub struct LnurlClient {
    ...
    lnurl_response: Arc<Mutex<Option<(PayResponse, Instant)>>>,
}
...
impl LnurlClient {
    ...
    pub async fn resolve_without_timeout(&self, amount_fiat: i64) -> Result<Invoice, String> {
        let (invoice, verify) = resolve_amount_with_currency_code(
            ...
            fetch_lnurl_response(self.lnurl_response.clone(), self.lnurl.clone()).await?,
...
async fn fetch_lnurl_response(
    cache: Arc<Mutex<Option<(PayResponse, Instant)>>>,
    endpoint: String,
    ...
    let value = fedimint_lnurl::request(&endpoint)
        .await
